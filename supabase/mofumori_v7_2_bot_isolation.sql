-- Keep arena CPU profiles out of public leaderboards.
...
declare
  ...
  me_json jsonb := null;
begin
  if board_name not in ('total','collection','social','explorer') then board_name:='total'; end if;
  ...
  pet_stats as (
    ...
    from public.mofumori_pets where owner_key not like 'bot:arena:%' group by owner_key
  ),
  friend_stats as (
    select owner_key,count(*)::integer friend_count
    from public.mofumori_friendships
    where owner_key not like 'bot:arena:%' and friend_key not like 'bot:arena:%'
    group by owner_key
  ),
  visit_stats as (
    ...
    from public.mofumori_visits
    where visitor_key not like 'bot:arena:%'
  ...
  action_stats as (
    ...
    from public.mofumori_visit_actions
    where actor_key not like 'bot:arena:%'
    group by actor_key
  ),
  base as (
    ...
    where p.user_key not like 'bot:arena:%'
  ),
  scored as (
    select *,
      case board_name
        ...
        else collection_score+social_score+explorer_score
      end leaderboard_score
    from base
  ),
  ranked as (
    select *,dense_rank() over(order by leaderboard_score desc,player_id asc)::integer position
    from scored
  ),
  limited as (select * from ranked order by position asc,player_id asc limit row_limit)
  select coalesce(jsonb_agg(jsonb_build_object(
    'position',position,'playerId',player_id,'displayName',display_name,'score',leaderboard_score,
    'character',character,'isMe',user_key=p_user,
