...
async function userEvents(sb,userKey,resolveLifecycle){
  ...
  try{await resolveLifecycle?.(sb,{id:userKey})}catch(e){console.warn('[push] lifecycle resolve skipped',e?.message||e)}
...
async function dispatch(sb,resolveLifecycle){
  ...
  for(const [userKey,userSubs] of groups){
    const events=await userEvents(sb,userKey,resolveLifecycle);eventCount+=events.length;
...
module.exports=async function handler(req,res,options={}){
  ...
  try{
    ...
    if(action==='dispatch'){
      ...
      const result=await dispatch(sb,options.resolveLifecycle);
