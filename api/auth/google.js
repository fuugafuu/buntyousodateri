...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    res.setHeader('set-cookie', `${COOKIE_NAME}=${encodeURIComponent(body.credential)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${Math.max(60, Math.floor((user.expiresAt - Date.now()) / 1000))}${secure}`);
