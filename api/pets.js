...
const ADMIN_RARITIES=['N','R','SR','SSR','UR'];
function adminNumber(value,min,max){
  const n=Number(value);
  if(!Number.isFinite(n)||n<min||n>max)throw Object.assign(new Error('数値設定が範囲外です。'),{status:400});
  return n;
}
function validateAdminGachaConfig(input){
  if(!input||typeof input!=='object'||!Array.isArray(input.banners)||input.banners.length<1||input.banners.length>12){
    throw Object.assign(new Error('ガチャ設定が不正です。'),{status:400});
  }
  const ids=new Set();
  const banners=input.banners.map(raw=>{
    const id=text(raw?.id,32).toLowerCase();
    if(!/^[a-z0-9_-]{2,32}$/.test(id)||ids.has(id))throw Object.assign(new Error('ガチャIDが不正または重複しています。'),{status:400});
    ids.add(id);
    const rates={};let rateSum=0;
    for(const rarity of ADMIN_RARITIES){
      rates[rarity]=adminNumber(raw?.rates?.[rarity],0,100);
      rateSum+=rates[rarity];
    }
    ...
    for(const [species,value] of Object.entries(raw?.speciesWeights||{})){
      if(!GACHA_SPECIES.has(species))continue;
      const weight=adminNumber(value,0,10000);
      if(weight>0)speciesWeights[species]=weight;
    }
    if(!Object.keys(speciesWeights).length)throw Object.assign(new Error('排出対象のどうぶつを1種類以上設定してください。'),{status:400});
    return {
      id,
      name:text(raw?.name,40,id),
      enabled:raw?.enabled!==false,
      price1:Math.round(adminNumber(raw?.price1,0,100000)),
      price10:Math.round(adminNumber(raw?.price10,0,100000)),
      rates,
      speciesWeights
    };
  });
  let activeBanner=text(input.activeBanner,32).toLowerCase();
  if(!ids.has(activeBanner)||!banners.find(b=>b.id===activeBanner)?.enabled){
    activeBanner=banners.find(b=>b.enabled)?.id||banners[0].id;
  }
  return {activeBanner,banners};
}
async function auditAdmin(sb,user,action,target,detail={}){
  try{
    const {error}=await sb.from('mofumori_admin_audit').insert({admin_key:user.id,action,target:target||null,detail});
    if(error)console.warn('[admin-audit]',error.message);
  }catch(error){console.warn('[admin-audit]',error?.message||error)}
}
async function loadAdminGachaConfig(sb){
  const {data,error}=await sb.from('mofumori_game_config').select('value,updated_at,updated_by').eq('key','gacha').maybeSingle();
  if(error)throw error;
  return data||null;
}
async function handleAdminAction(sb,user,payload){
  const admin=await requireAdmin(sb,user);
  const action=text(payload?.adminAction,32);
  if(action==='status'){
    const config=await loadAdminGachaConfig(sb);
    return {authorized:true,role:admin.role,permissions:admin.permissions,gacha:config?.value||DEFAULT_GACHA_CONFIG,updatedAt:config?.updated_at||null};
  }
  if(action==='setGachaConfig'){
    if(admin.permissions?.gacha!==true)throw Object.assign(new Error('ガチャ設定の権限がありません。'),{status:403});
    const value=validateAdminGachaConfig(payload.config);
    const {error}=await sb.from('mofumori_game_config').upsert({
      key:'gacha',value,updated_by:user.id,updated_at:new Date().toISOString()
    },{onConflict:'key'});
    if(error)throw error;
    await auditAdmin(sb,user,'set_gacha_config',value.activeBanner,{bannerCount:value.banners.length});
    return value;
  }
  if(action==='adjustCurrency'){
    if(admin.permissions?.economy!==true)throw Object.assign(new Error('通貨操作の権限がありません。'),{status:403});
    const playerId=text(payload.playerId,16).toUpperCase();
    if(!/^MF-[A-Z0-9]{8,12}$/.test(playerId))throw Object.assign(new Error('プレイヤーIDが不正です。'),{status:400});
    const coins=Math.trunc(adminNumber(payload.coinsDelta||0,-100000000,100000000));
    const gems=Math.trunc(adminNumber(payload.gemsDelta||0,-1000000,1000000));
    const {data,error}=await sb.rpc('mofumori_admin_adjust_currency',{
      p_admin:user.id,p_player_id:playerId,p_coins:coins,p_gems:gems
    });
    if(error){
      const message=String(error.message||'');
      if(message.includes('player_missing'))throw Object.assign(new Error('対象プレイヤーが見つかりません。'),{status:404});
      if(message.includes('save_missing'))throw Object.assign(new Error('対象プレイヤーのクラウドセーブがまだありません。'),{status:409});
      throw error;
    }
    await auditAdmin(sb,user,'adjust_currency',playerId,{coinsDelta:coins,gemsDelta:gems});
    return data;
  }
  if(action==='diagnostics'){
    ...
    return {profiles:profiles||0,pets:pets||0,activeMatches:matches||0,arenaBots:bots||0,serverTime:new Date().toISOString()};
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    if(req.method==='GET'){
      const url=new URL(req.url||'/api/pets','http://localhost');
      if(url.searchParams.get('config')!=='gacha')return json(res,404,{ok:false,message:'設定が見つかりません。'});
      if(!configured())return json(res,200,{ok:true,data:DEFAULT_GACHA_CONFIG,configured:false});
      const supabase=getSupabase(),data=await loadGachaConfig(supabase);
      return json(res,200,{ok:true,data,configured:true});
    }
    ...
    if(action==='admin'){
      const data=await handleAdminAction(supabase,user,payload);
    ...
    return json(res, error.status || 400, { ok: false, authorized:![401,403].includes(error.status), message: error.message || 'オンライン機能の処理に失敗しました。' });
  }
};