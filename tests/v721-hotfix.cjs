...
const config=fs.readFileSync('server/config-api.cjs','utf8');
...
assert.ok(/^7\./.test(pkg.version),'current v7 release must remain compatible with v7.2.1+ guards');
...
assert.ok(vercel.rewrites.some(r=>r.source==='/api/admin'&&r.destination.includes('__route=admin')),'admin rewrite missing');
assert.ok(vercel.rewrites.some(r=>r.source==='/api/config'&&r.destination.includes('__route=config')),'config rewrite missing');
assert.ok(pets.includes("route==='admin'")&&pets.includes("route==='config'"),'shared API router missing');
