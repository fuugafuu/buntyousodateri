...
assert.ok(/^7\./.test(pkg.version),'current v7 release must remain compatible with v7.2.3+ guards');
assert.ok(html.includes(`window.MOFUMORI_BUILD='${pkg.version}'`));
assert.ok(html.includes(`daily-events.css?v=${pkg.version}`)&&html.includes(`daily-events.js?v=${pkg.version}`));
assert.ok(sw.includes(`mofumori-shell-v${pkg.version}`)&&sw.includes(`/daily-events.js?v=${pkg.version}`));
