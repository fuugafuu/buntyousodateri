Original prompt: 日本語で作業してください。まず本気で全体のバグ修正とシステムの異常、さらにテクスチャとUIの一新。ミニゲームの整理と追加。本気で改良し、最後にURLを出す。GitHub側を編集する。

追加依頼: `E:\qwen2.5-1.5b-instruct-q4_k_m.gguf` をスマホ端末内で読み込み・キャッシュし、AIの要求・表情・自発吹き出しを追加。育成の楽しさ、ランキング、プレイヤーIDフレンド、文鳥閲覧、アイテム仕送り、Googleログイン、クラウド保存を追加し、公開サイトを復旧する。

## 作業メモ

- 対象: `fuugafuu/buntyousodateri` の `codex/add-feature-to-name-animations-and-birds` を基点にした `codex/full-game-refresh`。
- 既存の猫・きつね・ペンギン・隠しキャラふうが・ミッション報酬を保持しながら全体改修する。
- 2026-07-17: 文鳥専用Next.js版を誤って対象にしたため、公開前に停止。正しい `codex/full-game-refresh` から `codex/mofumori-ai-social` を作成して再開。

## 追加TODO

- [x] Cookie分割・localStorage多重保存をIndexedDB＋クラウド同期へ移行
- [x] GGUF端末AI、OPFSキャッシュ、表情、要求、自発吹き出し
- [x] Google Identity Servicesログイン
- [x] 絆ランク、フレンド、ランキング、仕送り
- [x] 正しいMofumoriを実モデル・API・モバイルで再検証
- [x] 正しいブランチをGitHub/Vercelへ公開

## 2026-07-17 追加実装メモ

- ゲーム進行の保存先を `mofumori-v4` IndexedDBへ変更。旧Cookie/localStorageは初回移行の読み取り専用になり、移行後に削除する。
- セーブJSONから旧ChatGPT APIキー項目を除外し、UIからAPIキー入力を撤去。
- 正しいMofumoriへwllama 3.5.1ブラウザビルドと単一スレッドWASMを同梱。
- GGUF利用条件、OPFS保存、WebGPU/WASM起動、端末AIチャット、表情、要求ボタン、自発吹き出しを追加。
- `node --check main.js` 成功。WebゲームクライアントでMofumori本体と `localAI` 状態JSONを確認。
- Google IDトークンをサーバーで検証するHttpOnly Cookie認証、クラウドセーブAPI、フレンド・ランキング・仕送りAPIを追加。
- Supabase用スキーマと、仕送り送信／受け取りを二重処理させないPostgresトランザクション関数を追加。
- プレイヤーID、なかよし度、連続お世話、育成デイリー、フレンドのどうぶつ閲覧、モバイルUIを追加。未ログイン時は端末モードで試せる。
- PWAマニフェスト、Service Worker、ホーム画面アイコンを追加。GGUF本体は通常キャッシュと分離してOPFSへ保持する。
- 保存データ由来の持ち物キーを許可リスト化し、動的なフレンド／仕送りボタンからインラインJavaScript生成を除去。APIの同一オリジン検査とセキュリティヘッダーを追加。

## 2026-07-17 検証結果

- 実モデル `E:\qwen2.5-1.5b-instruct-q4_k_m.gguf`（1,117,320,736 bytes）をWebGPUで読み込み成功。
- 実推論結果: 「ぼくの今の気持ちが😊です。どんなことをしてほしいですか？」、`ai-happy` 表情、推論回数1を確認。
- 端末AIのロード直後の自発推論とユーザー送信が競合する不具合を再現し、ユーザー送信優先へ修正。
- モバイル390×844でフレンド追加、相手のどうぶつ閲覧、シード仕送り、IndexedDB再読込、なかよし度増加、空腹時のAI要求→えさ実行を確認。
- 14種類のミニゲームすべてで開始UI、稼働状態、終了処理を再検証。コンソールエラー0。
- `npm test`、`npm run test:ui`、`npm audit --omit=dev`（脆弱性0件）成功。
- PR #6を`main`へマージ。Vercel本番デプロイがREADYとなり、`buntyousodateri.vercel.app`へエイリアス接続された。

## TODO

- [x] 現状のUIと全機能をブラウザで診断
- [x] 状態管理・保存・ミッション・ショップ・お世話の不具合修正
- [x] テクスチャとUIを一新
- [x] ミニゲームを整理し、新規ゲームを追加
- [x] Node構文確認とPlaywright実機検証
- [x] GitHubへ反映し、公開URLを確認

## 実装済み

- セーブ値の型・範囲・動物ID・持ち物・ミッションを正規化し、壊れた保存データでも復旧できるようにした。
- 開きっぱなし時の状態減少と病気発生率を現実的な速度へ修正。
- 水浴びによるシャンプー自動消費を廃止。水浴びは部分回復、持ち物のシャンプーは全回復に整理。
- 残留ミニゲームタイマーを一括破棄し、終了後のDOM再描画エラーを防止。
- 重複していた追加ミニゲーム30件を廃止し、カテゴリ付き14種へ整理。「ごはん仕分け」「おうちへ帰ろう」を新規追加。
- 猫・きつね・ペンギン・ふうがを個別SVGモデルへ刷新。室内テクスチャとレスポンシブUIを追加。
- オフライン会話を追加し、APIなしでもおしゃべり可能にした。チャット/エラーログのHTMLをエスケープ。
- `render_game_to_text`、`advanceTime`、Fキー全画面表示を追加。

## 検証メモ

- `node --check main.js`: 成功。
- Webゲーム用Playwrightクライアント: 初回画面、状態JSON、コンソールエラーなしを確認。
- 初回画面: `output/refresh-1/shot-0.png`。
- 14種類すべてのミニゲームで起動・主要操作・終了を検証。残留タイマー0、コンソールエラー0。
- 新規「ごはん仕分け」「おうちへ帰ろう」をプレイ中の画面まで目視確認。
- PC 1365x900、低いPC画面 1280x720、スマホ 390x844 でレスポンシブ表示を確認。
- ミッション5件目のダイヤ+2、壊れたセーブ値の正規化、チャットHTMLエスケープ、睡眠時間の入力検証を確認。
- GitHub PR #4を作成し、公開URLでHTTP 200・ゲームDOM・状態API・コンソールエラー0を確認。

## 2026-07-17 アカウントセーブ修正

- ゲストセーブとGoogleアカウント別セーブをIndexedDB内で分離。ログイン時にゲストの自動保存日時がクラウドデータを上書きする競合を解消。
- クラウドに既存データがある初回ログインは、端末のゲスト日時ではなくアカウントデータを優先。コイン、ダイヤ、持ち物、レベル、動物をまとめて画面へ反映。
- クラウドにデータがない場合は現在の育成データをアカウントへ初期登録。ログアウト時はゲストデータへ戻す。
- クラウド未設定時もGoogleアカウント別の端末セーブへ分離し、同期済みと誤表示しないよう文言を修正。
- Service Workerのキャッシュ名を更新し、公開後に古い`main.js`/`social.js`が残る問題を防止。
- `tests/ui-account-sync.mjs`で、リモートの9876コイン・42ダイヤ・シード77個の読込、ゲスト321コインの維持、アカウント側だけ12345コインへ更新されることを確認。
- `npm test`、`npm run test:ui`、Webゲーム用Playwrightクライアントを完走。14ミニゲームとフレンド機能の回帰なし。
- 公開API `/api/health` は現時点で `cloudConfigured:false`。Supabase環境変数が未登録のため、端末間クラウド同期にはVercel側の`SUPABASE_URL`と`SUPABASE_SERVICE_ROLE_KEY`登録が必要。
- 未ログイン時の`/api/auth/me`を正常なゲスト応答へ変更し、公開版で不要な401コンソールエラーが出ないようにした。

## 2026-07-17 ログイン導線改善

- スマホでも常時見えるヘッダー右上へ、青い「ログイン／データ同期」ボタンを追加。
- 下部の横スクロールメニューでは「ログイン・同期」を2番目へ移動し、隠れにくくした。
- どちらの入口からでも「みんなの森」のアカウント欄へ自動スクロールする。
- ログイン後はヘッダーを緑の「アカウント／同期状態」表示へ切り替える。
- 下部メニューの横スクロールを廃止し、スマホでは8機能を4列×2段で常時表示。アイコンと短い名称を分けて読みやすくした。
- ショップなど開いている機能は、緑の強調色・白枠・`aria-pressed`で選択状態が分かるようにした。
- スマホでは2段メニューを画面下へ固定し、パネル内へ移動した後もショップ・持ち物などへ直接切り替えられるようにした。
