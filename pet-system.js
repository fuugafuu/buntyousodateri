...
function renderAdminTab(tab){
  ...
  if(tab==='economy'){
    ...
    document.getElementById('adminApplyCurrency').onclick=async()=>{try{const d=await adminApi('adjustCurrency',{playerId:document.getElementById('adminCurrencyPlayer').value,coinsDelta:Number(document.getElementById('adminCoinsDelta').value||0),gemsDelta:Number(document.getElementById('adminGemsDelta').value||0)});document.getElementById('adminCurrencyResult').textContent=JSON.stringify(d,null,2);showToast('通貨を更新しました','achievement')}catch(e){showToast(e.message,'warning')}};return;
  }
  if(tab==='diag'){
...
function renderAdminGacha(root){
  const cfg=typeof structuredClone==='function'?structuredClone(N.admin.gacha):JSON.parse(JSON.stringify(N.admin.gacha||N.gachaConfig||{activeBanner:'standard',banners:[]}));N.adminEdit=cfg;
  const banners=cfg.banners||[];let selected=cfg.activeBanner||banners[0]?.id||'';const b=banners.find(x=>x.id===selected)||banners[0];
  root.innerHTML=`<section class="v721-admin-card"><div class="v721-admin-row"><h3>✨ ガチャ管理</h3><button id="adminAddBanner">＋ 新ガチャ</button></div><label>編集するガチャ<select id="adminBannerSelect">${banners.map(x=>`<option value="${escapeHtml(x.id)}" ${x.id===b?.id?'selected':''}>${escapeHtml(x.name||x.id)}</option>`).join('')}</select></label><div id="adminBannerEditor"></div><button id="adminSaveGacha">サーバーへ保存</button></section>`;
  const renderEditor=()=>{const id=document.getElementById('adminBannerSelect').value,x=cfg.banners.find(y=>y.id===id);if(!x)return;const rates=x.rates||{};document.getElementById('adminBannerEditor').innerHTML=`<label>表示名<input id="abName" value="${escapeHtml(x.name||'')}"></label><label class="v721-admin-check"><input id="abEnabled" type="checkbox" ${x.enabled!==false?'checked':''}> 有効</label><div class="v721-admin-grid"><label>1回価格<input id="abP1" type="number" value="${Number(x.price1||0)}"></label><label>10回価格<input id="abP10" type="number" value="${Number(x.price10||0)}"></label></div><h4>レア度確率（合計100%）</h4><div class="v721-admin-rates">${['N','R','SR','SSR','UR'].map(k=>`<label>${k}<input data-rate="${k}" type="number" step="0.1" value="${Number(rates[k]||0)}"></label>`).join('')}</div><label>排出ウェイト(JSON)<textarea id="abWeights" spellcheck="false">${escapeHtml(JSON.stringify(x.speciesWeights||{},null,2))}</textarea></label><button id="adminDeleteBanner" class="danger">このガチャを削除</button>`;document.getElementById('adminDeleteBanner').onclick=()=>{if(cfg.banners.length<=1)return showToast('最低1つは必要です','warning');cfg.banners=cfg.banners.filter(y=>y.id!==id);if(cfg.activeBanner===id)cfg.activeBanner=cfg.banners[0].id;N.admin.gacha=cfg;renderAdminGacha(root)}};
  const capture=()=>{const id=document.getElementById('adminBannerSelect')?.value,x=cfg.banners.find(y=>y.id===id);if(!x)return;x.name=document.getElementById('abName')?.value||x.name;x.enabled=document.getElementById('abEnabled')?.checked!==false;x.price1=Number(document.getElementById('abP1')?.value||0);x.price10=Number(document.getElementById('abP10')?.value||0);x.rates={};document.querySelectorAll('[data-rate]').forEach(el=>x.rates[el.dataset.rate]=Number(el.value||0));try{x.speciesWeights=JSON.parse(document.getElementById('abWeights')?.value||'{}')}catch(e){throw Error('排出ウェイトJSONが不正です')}cfg.activeBanner=id};
  document.getElementById('adminBannerSelect').onchange=()=>renderEditor();
  document.getElementById('adminAddBanner').onclick=()=>{const id='banner_'+Date.now().toString(36);cfg.banners.push({id,name:'新しいガチャ',enabled:true,price1:180,price10:1600,rates:{N:55,R:27,SR:13,SSR:4,UR:1},speciesWeights:{...W}});cfg.activeBanner=id;N.admin.gacha=cfg;renderAdminGacha(root)};
  document.getElementById('adminSaveGacha').onclick=async()=>{try{capture();const data=await adminApi('setGachaConfig',{config:cfg});N.admin.gacha=data;N.gachaConfig=data;N.gachaBanner=data.activeBanner;renderGachaHub();renderAdminGacha(root);showToast('ガチャ設定を保存しました','achievement')}catch(e){showToast(e.message,'warning')}};
  renderEditor();
}
