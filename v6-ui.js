...
function renderPetSheet(){
  ...
  const hero=stage==='egg'?`<section class="v6-pet-hero egg-mystery"><div class="v6-big-bird">🥚</div><div><small>UNKNOWN EGG</small><h3>なぞの卵</h3><div class="v6-tags"><span>ランク未公開</span><span>種類未公開</span><span>🥚 未孵化</span>${Number(p.generation||0)>0?'<span>G'+Number(p.generation)+'</span>':''}</div></div></section>`:`<section class="v6-pet-hero"><div class="v6-big-bird">${petIcon(p)}</div><div><small>${esc(p.speciesName||birds?.[p.species]?.name||'文鳥')}</small><h3>${esc(petName(p))}</h3><div class="v6-tags"><span>${p.rarity||'N'}</span><span>Rank ${p.rank||1}</span><span>${esc(lifeStageLabel(p))}</span><span>${esc(sexLabel(p))}</span>${Number(p.generation||0)>0?'<span>G'+Number(p.generation)+'</span>':''}</div></div></section>`;
  root.innerHTML=`${hero}
  ${stageCard}${sexCard}
  ${stage==='egg'?'':(Number(p.generation||0)>0||p.fatherId||p.motherId||familyChildren(p.id).length?geneticsSummary(p):'')}
  ${stage==='egg'?'':s?`<section class="v6-body-grid"><div><small>体重</small><b>${Number(s.weightG).toFixed(1)} g</b><em>適正 ${Number(s.idealWeightG).toFixed(1)} g</em></div><div><small>体長</small><b>${Number(s.bodyLengthCm).toFixed(1)} cm</b><em>翼幅 ${Number(s.wingSpanCm).toFixed(1)} cm</em></div><div><small>特徴</small><b>${esc(ph.personality||s.personality||'個性的')}</b><em>${esc(ph.sizeClass||'標準')} ・ 体格 ${Math.round(s.frame||0)}</em></div><div><small>対戦</small><b>${stage==='adult'?s.rating||1000:'--'}</b><em>${stage==='adult'?((s.wins||0)+'勝 '+(s.losses||0)+'敗 '+(s.draws||0)+'分'):'成鳥から参加可能'}</em></div></section>
...
function startPolling(){clearInterval(A.poll);A.poll=setInterval(()=>{if(!logged()||document.hidden)return;const arenaOpen=$('#v6Arena')?.classList.contains('show'),battleOpen=$('#v6Battle')?.classList.contains('show');if(arenaOpen||battleOpen)refreshArena()},battleOpen?900:2200)}
